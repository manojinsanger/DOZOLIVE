...
import FiltersBeautyModal from './Modals/FiltersBeautyModal';
...
const LiveOverlayOfLive: React.FC<LiveOverlayOfLiveProps> = ({
    ...
    const [beautyModalVissible, setBeautyModalVisible] = useState(false)
    const { user } = useAuth();
    const chatScrollRef = useRef<FlatList>(null);
    const scaleValue = new Animated.Value(1);

    const openBeautyModal = () => {
        setBeautyModalVisible(true);
        setLiveOptionModalVisible(false);
    }
        ...
        >
            <View style={{ flex: 1, position: 'relative',}}>
                ...
                <LiveOptionsModal
                    ...
                    openBeautyModal={openBeautyModal}
                />
                <UserBroadCastWaitingModal
                    visible={broadCastWaitngModalVisible}
                    onClose={() => setBoradCastWaitingModalVisible(false)}
                />
                <FiltersBeautyModal visible={beautyModalVissible} onClose={() => setBeautyModalVisible(false)} />
            </View>
        </KeyboardAvoidingView>
    );
};
