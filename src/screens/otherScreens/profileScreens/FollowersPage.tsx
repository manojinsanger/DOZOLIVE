...
const FollowersPage = ({ onClose }: Props) => {
  ...
  const { getFollowers, isLoading, } = useFollow();
  ...
  const loadFollowing = async (refresh = false) => {
    ...
    try {
      const res = await getFollowers(null, trimmedQuery || null, newPage, LIMIT);
      const fetchedData = res?.users;
      // console.log("followers: ", fetchedData)
      if (refresh) {
        setFollowers(fetchedData || []);
  ...
  const renderFooter = () => {
    if (!isLoadingMore || following.length === 0) return null;
