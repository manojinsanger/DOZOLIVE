...
const FollowingPage = ({ onClose }: Props) => {
  ...
  const { getFollowing, isLoading, } = useFollow();
  ...
  const loadFollowing = async (refresh = false) => {
    ...
    try {
      const res = await getFollowing(null, trimmedQuery || null, newPage, LIMIT);
      const fetchedData = res?.users;
      if (refresh) {
        setFollowing(fetchedData || []);
