...
import AcceptButton from '@/components/agent/AcceptButton';
import AcceptedButton from '@/components/agent/AcceptedButton';
...
const ActivityCenter = () => {
  ...
  const [isAccepted, setIsAccepted] = useState(false); // New state for footer button
  ...
  const DateFilterComponent = ({ activeTab, onTabPress }: { activeTab: string; onTabPress: (tab: string) => void }) => (
    <TouchableOpacity
    ...
    <MainContainer>
      <View style={styles.container}>
        ...
        <ScrollView>
          <View style={styles.activityContainer}>
            <View style={styles.rowFilterWrapper}>
              <View>
                <FlagDropdown countries={countries} selected={selected} onSelect={(c: any) => setSelected(c)} />
              </View>
              <DateFilterComponent activeTab={filterTab} onTabPress={setFilterTab} />
            </View>
                ...
                <View style={styles.footerTextContainer}>
                  <Text style={styles.footerText}>
                    ...
                    {filteredActivityData.length > 0
                      ? parseInt(filteredActivityData[0].points.replace(/,/g, '')) - 30285524
                      : 0}
                  </Text>
                  <Text style={styles.footerNote}>You can only receive rewards for the next rank, if the</Text>
                </View>
                {isAccepted ? <AcceptedButton /> : <AcceptButton onPress={handleAccept} />}
              </View>
            </View>
            <ReceiveButton />
          </View>
        ...
        <LiveDataDescriptionModal visible={showModal} onClose={() => setShowModal(false)}>
          <Text>1. Ranked by regional agency revenue, the total revenue for all days in the period will be counted</Text>
          <Text>2. The ranking is based on the total revenue of the agent in the period</Text>
          <Text>3. Only income of the host in the ranking region will be counted (Platform rewards excluded).</Text>
          <Text>
            4. Agents need to manually claim the rewards the day after the end of the cycle. If the agent fails to claim
            the rewards on time, the reward will be automatically cancelled.
          </Text>
